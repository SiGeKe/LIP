...
void main()
{
    ...
    printf("Defined Variables (2/3).\n");

    if (dlipc != 1 && nr_types == 2) printf("Error: Too many types of lipids!\n");
    if (t_total%(nr_data*nr_count) != 0) printf("Error: Wrong time given!\n");
    if (nr_count > t_total/nr_data) printf("Error: nr_count too high!\n");
...
void calc_all()
{
    ...
    double alpha = 0.05;
    double last_diff = 1000000000000000;
    ...
    while (0 == 0){
        ...
        diff = order_diff()*100000;
        plot_Entro(run,ufname);
        ...
        if (diff <= 1.3) printf("Achtung! Guter Wert bei %lf\n", run);


        alpha = (diff/10000)*alpha_factor;
        ...
        printf("Run: %d, Difference: %lf, Alpha: %lf\n", run, diff, alpha);
...
void calc_evolution()
{
    ...
    for (n=0;n<=nr_average;n++){
        ...
        for (t=1;t<=t_total;t++){
            ...
            if (t%(t_total/(nr_data*nr_count)) == 0){
            ...
            for (l=0;l<nr_lattice;l++){
                ...
                mc_move_order(lm);
                mc_move_particle(lm);
                if (cholpos[l] == 1) mc_move_chol(lm);
            }
        }
...
void make_interaction()
{
    ...
    if (inter_type == 1){
        if (dlipc == 1 && nr_types == 1){
            ...
            for (s=0;s<=300;s++){
                for (b=0;b<=6;b++) H_dd_avg[1][1][s][b] = H_DD_LL_avg(s,b);
            }
        }
        if (dlipc != 1 && nr_types == 1){
            for (a=0;a<=4;a++){
            ...
            for (s=0;s<=300;s++){
                for (b=0;b<=6;b++) H_dd_avg[1][1][s][b] = H_DD_PP_avg(s,b);
            }
        }
...
double ener_dd(int l, int m)
{
    ...
    double alpha = avg_part(max_order(l,x));
...
double ener_self(int l)
{
    ...
    return (H_self[i][order[l]] - temperature*Entro[i][order[l]]);
}
